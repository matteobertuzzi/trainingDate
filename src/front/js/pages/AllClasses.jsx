...
export const AllClasses = () => {
    ...
    const { postUserClass, deleteUserClass, setActiveNavTab } = actions;
            ...
            <Container className="min-vh-100 my-2">
                <Row className="d-flex justify-content-center align-items-center">
                    <Col lg={8} md={10} sm={10} xs={10} className="d-flex flex-column p-3 w-auto">
                        <div className="border rounded p-4 d-flex justify-content-center align-items-center" style={{ boxShadow: 'inset 0 0 15px rgba(255, 165, 0, 0.5)' }}>
                            <h4>Descubre todas las clases disponibles!</h4>
                        </div>
                    </Col>
                </Row>
                <Row className="d-flex justify-content-center align-items-center mt-2">
                    {(!userClasses || userClasses.length === 0 || !currentClasses.some(cc => !userClasses.some(uc => uc.trainer_class.class_details.id === cc.class_details.id))) ? (
                        ...
                        currentClasses.map((oneClass) => (
                            !userClasses.some(userclass => userclass.trainer_class.class_details.id === oneClass.class_details.id)
                                ? (
                                        ...
                                        <Card className="d-flex flex-column">
                                            <div className="position-relative">
                                                ...
                                                <Card.ImgOverlay style={{ position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 1 }}>
                                                    <span className="text-white">{oneClass.specialization.name}</span>
                                                </Card.ImgOverlay>
                                            </div>
